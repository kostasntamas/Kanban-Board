...
function apiLogin(): array {
    ...
    $stmt->execute([$_SESSION['user_id']]);
    $u = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$u) { http_response_code(401); die(json_encode(['error' => 'Unauthorized'])); }
    return $u;
}

function apiWorkspace(array $user): int {
    if (!isset($_SESSION['workspace_id'])) {
        http_response_code(403); die(json_encode(['error' => 'No workspace selected']));
    }
    global $pdo;
    $stmt = $pdo->prepare(
        "SELECT 1 FROM workspace_members WHERE workspace_id = ? AND user_id = ?"
    ...
    $stmt->execute([$_SESSION['workspace_id'], $user['id']]);
    if (!$stmt->fetch()) {
        http_response_code(403); die(json_encode(['error' => 'Not a member of this workspace']));
    }
    return (int) $_SESSION['workspace_id'];
}
